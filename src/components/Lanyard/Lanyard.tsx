...
function Band({
  ...
  // Match the 4:1 tile to the projected three-unit rope, keeping Marks square
  // across desktop/mobile aspect ratios without changing the physics.
  const bandRepeats = (3 * camera.projectionMatrix.elements[5] * size.height * (isMobile ? 0.5 : 1))
    / (size.width * lanyardWidth * 4);
  ...
  // Source images are composited on CPU; avoid eagerly uploading the 5906px
  // originals to the GPU. Hooks remain unconditional; use a blank pixel when an image
  ...
  const cardMap = useMemo(() => {
    ...
    const drawFitted = (img: TextureImage, rect: typeof FRONT_UV_RECT) => {
      const rx = rect.x * W;
      const ry = rect.y * H;
      const rw = rect.w * W;
      const rh = rect.h * H;
      const pick = imageFit === 'contain' ? Math.min : Math.max;
      // Keep artwork inside the rounded card face, clear of UV edge seams.
      const inset = imageFit === "contain" ? 0.82 : 1;
      const scale = pick(rw / img.width, rh / img.height) * inset;
    ...
    if (frontImage && frontTex.image) drawFitted(frontTex.image as TextureImage, FRONT_UV_RECT);
