...
import PurchaseReturnLink from "./PurchaseReturnLink";
...
export default function DeepCustomizationPage() {
  const destinations = getOfferingPackages().map(({ product, packages }) => ({
    product: product.slug,
    href: getOfferingNavigation(product).purchaseHref,
    packages: packages.map(bundle => bundle.id),
  }));
  return <div className={styles.page}><div className={styles.container}><nav aria-label="面包屑"><Link href="/">首页</Link> / 深度定制</nav>
    <header><p className={styles.eyebrow}>DEEP CUSTOMIZATION</p><h1>超出标准配置，<br/>进入工程级定制。</h1><p>ONE-G 优先通过标准产品与标准配置满足需求。对于特殊机械、电气、软件和机器人集成需求，可进入深度定制评估。</p><Suspense fallback={<Link href="/products">← 返回购买</Link>}><PurchaseReturnLink destinations={destinations}/></Suspense></header>
    ...
    <section><h2>提交深度定制需求</h2><p>登录后填写深度定制需求，当前仅支持前端预览。</p><SupportButton label="提交深度定制需求" entry="deep" inline/></section>
  </div></div>;
}
